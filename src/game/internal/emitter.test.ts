...
describe('EventEmitter', () => {
  ...
  test('warns (without throwing) when a cascade exceeds the configured depth', () => {
    ...
    const emitter = new EventEmitter({ warnDepth: 3, warn: (message) => warnings.push(message) });
  ...
  test('warnDepth 0 disables the guard entirely', () => {
    const warnings: string[] = [];
    const emitter = new EventEmitter({ warnDepth: 0, warn: (message) => warnings.push(message) });
